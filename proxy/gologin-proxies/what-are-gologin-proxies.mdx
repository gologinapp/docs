...
- **Dedicated IP** - one IP address reserved for you alone. It never changes, comes from a real internet provider, and has no traffic limit.

Residential, mobile, and datacenter proxies are paid for by traffic (in gigabytes). A Dedicated IP is paid for separately, as a \$5/month subscription per IP, and is available on paid plans.

<Note>
  **Dedicated IP**, **ISP proxy**, and **static proxy** all mean the same thing. The Gologin app calls it **Buy dedicated IP**, and that's the term used across this documentation.
</Note>

## About rotating proxies

Gologin residential and mobile proxies automatically rotate IP addresses every 5-30 minutes. This means your IP will change during a browsing session.

**Rotation is fine for:**

- Web scraping
- Short browsing sessions
- General research **When not to use rotating proxies**

<Warning>
  Do not use rotating proxies for Amazon, eBay, Etsy, PayPal, or any marketplace that monitors account history. Rotating IPs are one of the fastest ways to get accounts linked and banned on these platforms. Use a Dedicated IP for these accounts instead.
</Warning>
...
- LinkedIn logs out sessions and triggers security reviews when it sees an IP change between sessions. Rotating proxies will cause frequent logouts and verification prompts.
...
For long-term account management on any sensitive platform, you need one IP address per account that never changes. In Gologin, that's a **Dedicated IP**.

| Platform | Recommended proxy type |
| --- | --- |
| Amazon / eBay / Etsy | Dedicated IP |
| LinkedIn | Dedicated IP |
| Facebook / Meta Ads | Dedicated IP or static mobile |
| Betting sites | Static mobile |
| Web scraping | Rotating residential or datacenter |
| Testing / quick checks | Rotating Gologin proxies are fine |
