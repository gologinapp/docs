...
- LinkedIn, Facebook, and other social media accounts (IP changes trigger security checks)
- Amazon, eBay, Etsy seller accounts (IP changes can link/ban accounts)
- Any workflow requiring a stable, consistent IP

<Warning>
For account-based workflows, use a third-party static/ISP proxy. See [Recommended Proxy Providers](/proxy/proxy-management/recommended-proxy-providers).
</Warning>